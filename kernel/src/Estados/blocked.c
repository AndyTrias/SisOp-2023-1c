...
void wait(t_pcb *proceso)
{
    ...
    log_info(LOGGER_KERNEL,"PID: <%d> - Wait: <%s> - Instancias: <%s>",proceso->contexto.PID,RECURSOS[recurso_id],INSTANCIAS_RECURSOS[recurso_id]);
}
...
void signal(t_pcb *proceso)
{
    ...
    log_info(LOGGER_KERNEL,"PID: <%d> - Signal: <%s> - Instancias: <%s>",proceso->contexto.PID,RECURSOS[recurso_id],INSTANCIAS_RECURSOS[recurso_id]);
}